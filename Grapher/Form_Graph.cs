...
namespace BOT_V3.Grapher
{
    public partial class Form_Graph : Form
    {
        ...
        SignalPlotXY[] coinPrice_Close = new SignalPlotXY[13];
        ...
        int stepSize = 3;
        bool noGraph;
        public Form_Graph(string coinName, int interval_position, double RSI_limit, double getProfit, double stopLoss)
        {
            InitializeComponent();
        ...
        public Form_Graph(string coinName)
        {
            InitializeComponent();
            this.coinName = coinName;
        ...
        private void FP_CoinPrice_Load(object? sender, EventArgs e)
        {
            ...
            Invoke(new Action(delegate ()
            {
                #region Sql2Data
                //------------------------------------------------------------------------
                d_coinPrice = new double[intervals.Count(), 4][];
                //interval
                //time-high-low-close
                //counter
                for (int count_interval = 0; count_interval < intervals.Count(); count_interval++)
                {
                    ...
                    d_coinPrice[count_interval, 3] = new double[klinePriceData.Count];
                    ...
                    for (int counter_klinePriceData = 0; counter_klinePriceData < klinePriceData.Count; counter_klinePriceData++)
                    {
                        ...
                        d_coinPrice[count_interval, 3][counter_klinePriceData] = klinePriceData[counter_klinePriceData].Close_price;
                    }
                    ...
                    coinPrice_Close[count_interval] = FP_CoinPrice.Plot.AddSignalXY(d_coinPrice[count_interval, 0], d_coinPrice[count_interval, 3], color: Color.Purple);
                    coinPrice_High[count_interval].LineWidth = 1 + count_interval;
                    coinPrice_Low[count_interval].LineWidth = 1 + count_interval;
                    coinPrice_Close[count_interval].LineWidth = 1 + count_interval;
